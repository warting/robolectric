...
public class CppAssetManager2 {
  ...
  public boolean GetResourceName(int resid, ResourceName out_name) {
    ...
    ApkAssetsCookie cookie =
        FindEntry(
            resid, (short) 0 /* density_override */, true /* stop_at_first_match */, entryRef);
  ...
  ResolvedBag GetBag(int resid, List<Integer> child_resids) {
    ...
    ApkAssetsCookie cookie =
        FindEntry(
            resid, (short) 0 /* density_override */, false /* stop_at_first_match */, entryRef);
    if (cookie.intValue() == kInvalidCookie) {
      return null;
    }
    ...
    while (map_entry != null
        && map_entry.myOffset() != map_entry_end
    ...
    while (map_entry != null && map_entry.myOffset() != map_entry_end) {
      ...
      map_entry =
          new ResTable_map(
              map_entry.myBuf(),
              map_entry.myOffset() + map_entry.value.size + ResTable_map.SIZEOF - Res_value.SIZEOF);
      // ++new_entry;
      ++newEntryIndex;
    }
